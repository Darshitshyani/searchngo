...
export default function Login() {
  ...
  const [router_mac_address, setrouter_mac_address] = useState('');
  ...
  const handleLogin = async () => {
    ...
    if (window.location.pathname.split('/')[2] === 'pos') {
      let obj = { email: email, password: password, router_mac_address: router_mac_address, device_mac_address: device_mac_address }
    ...
    const { data } = await api.post(handleRoleAPI(), req);
    if (data) {
      const accessToken = data?.data?.token;
      localStorage.setItem("accessToken", accessToken);
      localStorage.setItem("user_role", data?.data?.user_data?.slug);
                ...
                <form className="login100-form validate-form m-auto">
                  ...
                  {window.location.pathname.split('/')[2] === 'pos' && <>
                    <div className="wrap-input100 validate-input">
                      <input
                        className="input100"
                        onChange={(e) => setrouter_mac_address(e.target.value)}
                        placeholder="Router mac address"
                      />
                      <span className="focus-input100"></span>
                      <span className="symbol-input100">

...
export default function PlantManger() {
  ...
  const getList = async () => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/plant-manager`);
    if (data) {
  ...
  const submitHandler = async () => {
    ...
    const { data } = await api.post(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/plant-manager`, new_formData);
  ...
  const updateData = async () => {
    ...
    const { data } = await api.put(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/plant-manager/${updateID}`, new_formData);
  ...
  const getInfoData = async (id) => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/plant-manager/${id}`);
    if (data) {

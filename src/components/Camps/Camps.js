...
export default function Camps() {
  ...
  const getList = async () => {
    const { data, error } = await api.get(`${baseUrl}/api/${ROLES_SLUG[_USER?.role]}/camps`);
    if (data) {
  ...
  const getDeletedList = async () => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/camps?status=0`);
    if (data) {
  ...
  const getcoordinatorList = async () => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/coordinator`);
    if (data) {
      let optionList = []
      for (let index = 0; index < data?.data?.list.length; index++) {
  ...
  const getaccountList = async () => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/pos`);
    if (data) {
      let optionList = []
      for (let index = 0; index < data?.data?.list.length; index++) {
        let obj = { value: data.data.list[index].id, label: data.data.list[index].full_name }
  ...
  const updateData = async () => {
    let { camps_uuid, client_id, status, ...removeExtraData } = modalData;
    let new_formData = {
      ...modalData,
      is_allowed_package_meal: `${(modalData?.is_allowed_package_meal === true || modalData?.is_allowed_package_meal === "1") ? "1" : "0"}`,
      is_allowed_package_water: `${(modalData?.is_allowed_package_water === true || modalData?.is_allowed_package_water === "1") ? "1" : "0"}`,
  ...
  const reActiveDeleted = async (row, value) => {
    const { data } = await api.post(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/camps/status-update/${row.id}`, { status: value === false ? "2" : "1" });
  ...
  const confirmDelete = async (deleteId) => {
    const { data } = await api.post(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/camps/status-update/${deleteId}`, { status: 0 });
  ...
  const assignHandle = async () => {
    ...
    const { data, error } = await api.post(`${baseUrl}/api/client-admin/camps/assign/${assignType}`, req);
    if (data) {

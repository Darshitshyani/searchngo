...
import CoordinatorViewModal from "../../Camps/ViewModal";
...
export default function ClientCoordinator() {
  ...
  const getActiveAssignPackagebyCamps = async (campIds) => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/camps/assigned-coordinators-by-camp?camp_id=${campIds}&status=1`);
    if (data) {
  ...
  const getDeactiveAssignPackagebyCamps = async (campIds) => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/camps/assigned-coordinators-by-camp?camp_id=${campIds}&status=0`);
    if (data) {
  ...
  const getActiveAssignCampsByCoordinator = async (coordinatorID) => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/coordinator/assigned-camps-by-coordinator?coordinator_id=${coordinatorID}&status=1`);
    if (data) {
  ...
  const getDeactiveAssignCampsByCoordinator = async (coordinatorID) => {
    const { data, error } = await api.get(`${baseUrl}/${ROLES_SLUG[_USER?.role]}/api/coordinator/assigned-camps-by-coordinator?coordinator_id=${coordinatorID}&status=0`);
    if (data) {
  ...
  const assignHandle = async () => {
    ...
    const { data, error } = await api.post(`${baseUrl}/client-admin/api/camps/assign-camps-to-coordinator`, req);
    if (data) {
      handleCloseModal()
      toast.success(data.message ? data.message : data.messages, {
        position: toast.POSITION.TOP_RIGHT,
                          ...
                          <Row className="mb-2">
                            ...
                            <div className="tabs-menu1 tabstyle2">
                              <Tabs
                                ...
                                onClick={(e) => tabchange(e)}
                              >
                                <Tab eventKey="ActiveCoordinatorWiseList" className="me-1 " title="Active Coordinator Wise List">
                                  <div className="table-responsive">
                                ...
                                <Tab eventKey="DeativeCoordinatorWiseList" className="me-1 " title="Deative Coordinator Wise List">
                                  <div className="table-responsive">
